...
############################################################################################################################################################
# This script queries and displays the Docker Hub Product pulls
#
#   Syntax: get_store_product_pulls.sh -p product-slug [-t {image|plugin}] -s start-date [-e end-date]
#
#   Pre-requisites:
#
...
function usage () {
    printf 'Usage: get_store_product_pulls.sh -p product-slug [-t {image|plugin}] -s start-date [-e end-date]\n' >&2
...
OUTPUT=$(curl -L --silent --header "Authorization: BEARER ${TOKEN}" \
--write-out \\n%{http_code} \
"https://store.docker.com/api//analytics/v1/reports/pulls?from=${START_DATE}T00:00:00.000Z&to=${END_DATE}T23:59:59.000Z&publisher_id=${PUBLISHER_ID}")
...
HTTP_CODE=$(echo "$OUTPUT" | awk 'END{print}')
if [[ "$HTTP_CODE" != "200" ]]; then
    printf 'Unable to get Product Pulls!\n' >&2
    print_api_error "${OUTPUT}"
    exit 1
fi
